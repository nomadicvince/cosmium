...
contract("Cosmium", (accounts) => {
  ...
  it("initializes contract with correct values", () => {
    return Cosmium.deployed().then((instance) => {
      ...
      assert.equal(name, "Cosmium", "has correct name");
      return tokenInstance.symbol();
    }).then((symbol) => {
      assert.equal(symbol, "CMQ", "has correct symbol");
      return tokenInstance.standard();
    }).then((standard) => {
      assert.equal(standard, "Cosmium v1.0", "has correct standard");
    })    
  });
